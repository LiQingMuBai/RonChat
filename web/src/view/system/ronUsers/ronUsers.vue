...
<template>
  <div>
   ...
   <template #default="scope">{{ formatDate(scope.row.deletedAt) }}</template>
            ...
            <el-table-column align="left" label="男女 1男 0女" prop="sex" width="120" />
            ...
            <el-table-column align="left" label="用户telegram" prop="telegram" width="120" />

            <el-table-column align="left" label="roomID" prop="roomId" width="120" />

        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateRonUsersFunc(scope.row)">编辑</el-button>
    ...
    {{ detailFrom.password }}
                    ...
                    <el-descriptions-item label="男女 1男 0女">
    {{ detailFrom.sex }}
</el-descriptions-item>
    ...
    {{ detailFrom.email }}
                    ...
                    <el-descriptions-item label="用户telegram">
    {{ detailFrom.telegram }}
</el-descriptions-item>
                    <el-descriptions-item label="roomID">
    {{ detailFrom.roomId }}
</el-descriptions-item>
            </el-descriptions>
        </el-drawer>
...
const getTableData = async() => {
  const table = await getRonUsersList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteRonUsersByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateRonUsersFunc = async(row) => {
    const res = await findRonUsers({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteRonUsersFunc = async (row) => {
    const res = await deleteRonUsers({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
